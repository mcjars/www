...
export default async function apiGetVersions(
  type: string,
  options?: ApiGetVersionsOptions,
): Promise<PaginatedResponse<MinecraftVersion>> {
  ...
  const { data } = await axios.get(
    `${BASE_URL}/api/v3/builds/types/${type.toUpperCase()}/versions?${params.toString()}`,
  );
  const versionsContainer =
    data?.versions && typeof data.versions === 'object' && !Array.isArray(data.versions) ? data.versions : undefined;
  ...
  const normalizedVersionList = (Array.isArray(versionList) ? versionList : []).map((v: any) => ({
    ...
    latest: {
      projectVersionId:
        v.latest?.projectVersionId ??
        v.latest?.project_version_id ??
        v.projectVersionId ??
        v.project_version_id ??
        null,
