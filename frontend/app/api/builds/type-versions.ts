...
export default async function apiGetTypeVersions(type: string): Promise<TypeVersionSummary[]> {
  ...
  return versions
    .map((version: any) => ({
      versionId: version.latest?.version_id,
      projectVersionId: version.latest?.project_version_id,
...
export async function apiGetTypeVersionBuilds(type: string, version: string): Promise<PartialMinecraftBuild[]> {
  const { data } = await axios.get(
    `${BASE_URL}/api/v3/builds/types/${type.toUpperCase()}/versions/${encodeURIComponent(version)}`,
  );
