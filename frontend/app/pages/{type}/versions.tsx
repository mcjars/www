...
export default function PageTypeVersions() {
  ...
  const filteredVersions = versions?.filter(
    (version) =>
      versionType === 'all' ||
      !versionType ||
      (versionType === 'stable' ? version.type === 'RELEASE' : version.type === 'SNAPSHOT'),
  );
  ...
  const recommendedVersionsPerPage = useMemo(() => {
    ...
    const columns = effectiveDisplayMode === 'grid' ? (mobile ? 1 : 2) : Math.max(1, compactColumns);
  ...
  const versionPageSizeOptions = useMemo(
    () =>
      Array.from(new Set([12, 16, 20, 24, 32, 40, 50, 100, 200, recommendedVersionsPerPage, versionsPerPage])).sort(
        (a, b) => a - b,
      ),
    ...
    <>
        ...
        <AlertDescription className={'flex flex-row items-center justify-between'}>
          <div className={'flex flex-row items-center'}>
            ...
            <div className={'flex flex-col justify-center'}>
              <h1 className={'text-2xl font-semibold flex flex-row items-center'}>{typeData?.name}</h1>
              <span className={'md:block hidden mt-1'}>
                <Badge
                  className={'mr-2'}
                  variant={typeData?.experimental || typeData?.deprecated ? 'destructive' : 'outline'}
                >
      ...
      {(typeData?.experimental || typeData?.deprecated) && (
        <Alert className={'mb-2'} variant={'destructive'}>
          <AlertDescription>
            Keep in mind, <span className={'font-semibold'}>{typeData?.name}</span> is{' '}
            {typeData?.experimental ? 'experimental' : 'deprecated'} and may not work as expected. Take backups!
          </AlertDescription>
        </Alert>
      )}
        ...
        {!filteredVersions ? (
          ...
          <>
            {filteredVersions.map((version) => (
              <Card
                key={version.latest.versionId ?? version.latest.projectVersionId}
                className={'mb-2 mx-1 p-3 pr-4 xl:min-w-[30rem]'}
              >
                <div className={'flex flex-row items-center justify-between'}>
                  <div className={'flex flex-row items-center'}>
                    <img
                      src={`https://s3.mcjars.app/icons/${type.toLowerCase()}.png`}
                      alt={'Logo'}
                    ...
                    <div className={'flex flex-col ml-2'}>
                      <h1 className={'text-xl font-semibold flex flex-row items-center'}>
                        ...
                        <Badge
                          ...
                          variant={version.type === 'RELEASE' ? 'outline' : 'destructive'}
                        >
                          {version.type}
                        </Badge>
                      ...
                      <p className={'text-sm text-gray-500'}>
                        {version.builds} Build{version.builds === 1 ? '' : 's'}, Java {version.java},{' '}
                  ...
                  <div className={'flex flex-row items-center'}>
                    <Button
                      variant={'outline'}
                      onClick={() => setBrowse(version.latest.versionId ?? version.latest.projectVersionId)}
                      disabled={Boolean(browse)}
                    >
            ...
            {!filteredVersions.length && <p className={'text-center text-gray-500'}>No versions found.</p>}
          </>
        )}
      </div>
                    ...
                    <Collapsible defaultOpen={i === 0} className={'group/collapsible-build'}>
                      <div className={'flex flex-row items-center justify-between'}>
                        <div className={'flex flex-row items-center'}>
                          ...
                          <div className={'flex flex-col ml-2'}>
                            <h1
                              className={'text-xl font-semibold flex md:flex-row flex-col md:items-center items-start'}
                            >
                              {build.name}
                            ...
                            <p className={'text-sm text-gray-500'}>
                              {bytes(
                                build.installation
                                  .flat()
                                  .filter((step) => step.type === 'download')
                                  .reduce((a, c) => a + c.size, 0),
                              )}
                              ,{' '}
                              {build.created
                                ? new Date(build.created).toLocaleDateString()
                                : `${build.changes.length} Change${build.changes.length === 1 ? '' : 's'}`}
                            </p>
                          </div>
                        </div>
                          ...
                          <CollapsibleTrigger>
                            <Button>
                              ...
                              <ChevronDown
                                size={16}
                                className={
                                  'ml-2 transition-transform group-data-[state=open]/collapsible-build:rotate-180'
                                }
                              />
                            </Button>
                          </CollapsibleTrigger>
                      ...
                      <CollapsibleContent className={'mt-2 flex flex-row items-end justify-between'}>
                        <div className={'flex flex-col'}>
                          {build.installation.flat().map((step, i) => (
                            <div key={i} className={'flex flex-row items-center mt-2'}>
                              ...
                              {step.type === 'remove' && (
                                <code className={'border rounded-md p-1 px-3 w-fit'}>{step.location}</code>
                              )}
                              {step.type === 'unzip' && (
                                <div className={'flex flex-row items-center'}>
                                  <code className={'border rounded-md p-1 px-3 w-fit'}>{step.file}</code>
                        ...
                        <div className={'w-3/5 hidden md:flex flex-row items-center'}>
                          ...
                          <Button
                            variant={'outline'}
                            className={'ml-2'}
                            onClick={() => setInstallScript((prev) => (prev === 'bash' ? 'mcvcli' : 'bash'))}
                          >
