    ...
    <div className={'w-full text-sidebar-foreground'}>
      {folderEntries.length > 0 && (
        <Accordion
          type={'multiple'}
          className={'w-full'}
          defaultValue={folderEntries.map(([name]) => `${path}/${name}`)}
        >
            ...
            return (
              <AccordionItem key={folderPath} value={folderPath} className={'border-none'}>
                <AccordionTrigger className={'px-0 py-0 hover:no-underline'}>
                  <div
                    className={
                      'flex w-full items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-sidebar-accent/60'
                    }
                  ...
                  >
                    <div className={'flex min-w-0 items-center gap-3'}>
                      <div
                        className={
                          'flex h-8 w-8 ml-2 items-center justify-center rounded-md border border-sidebar-border/80 bg-sidebar/60 text-sidebar-foreground/80'
                        }
                      >
                    ...
                    <div className={'mr-2 flex shrink-0 items-center gap-2'}>
                      <Badge variant={'secondary'} className={'bg-sidebar-accent/70 text-sidebar-accent-foreground'}>
                        {fileCount}
                      </Badge>
                    </div>
                  </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                selectedItem ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'hover:bg-sidebar-accent/60',
              )}
              style={{ paddingLeft: depth * 16 }}
            >
              <div className={'flex min-w-0 items-center gap-3'}>
...
export default function PageTypeConfig() {
  ...
  const typeData = useMemo(
    () =>
      Object.values(types ?? {})
        .flat()
        .find((entry: any) => normalizeTypeId(entry.identifier) === normalizeTypeId(type)),
    [types, type],
  );
      ...
      <div className={'mb-2 flex shrink-0 items-center justify-between gap-4'}>
        <div className={'min-w-0'}>
          ...
          <p className={'text-sm text-muted-foreground'}>
            {selectedBuildUuid
              ? `${fileCount} file${fileCount !== 1 ? 's' : ''} for selected build`
              : 'Select a version and build to view configs'}
          </p>
        </div>
        ...
        <div className={'grid w-full max-w-xl grid-cols-1 gap-2 sm:grid-cols-2'}>
          ...
          <Select value={selectedBuildUuid ?? undefined} onValueChange={setBuildParam}>
            <SelectTrigger className={'w-full'}>
              <div className={'flex min-w-0 items-center gap-2 text-left'}>
                {typeData?.icon ? (
                  <img src={typeData.icon} alt={typeData.name} className={'h-5 w-5 rounded-md'} />
                ) : null}
      ...
      {!types || !versionOptions || !buildItems || !configs ? (
        ...
        <div className={'flex min-h-0 flex-1 flex-col gap-2 overflow-hidden'}>
          <div className={'grid grid-cols-2 gap-2 lg:hidden'}>
            <Button variant={mobilePane === 'files' ? 'default' : 'outline'} onClick={() => setMobilePane('files')}>
              Files
            </Button>
            <Button variant={mobilePane === 'editor' ? 'default' : 'outline'} onClick={() => setMobilePane('editor')}>
              Editor
            </Button>
          </div>
          ...
          <div ref={layoutRef} className={'flex min-h-0 flex-1 flex-col gap-2 overflow-hidden lg:flex-row lg:gap-0'}>
            ...
            >
              <Card
                className={
                  'min-h-0 w-full overflow-hidden rounded-xl border-sidebar-border bg-sidebar p-0 text-sidebar-foreground flex flex-col lg:rounded-l-xl lg:rounded-r-none lg:border-r-0'
                }
              >
              ...
              {!isMobile && (
                <div
                  ...
                  className={
                    'absolute right-0 top-0 hidden h-full w-2 translate-x-1/2 cursor-col-resize rounded-full bg-transparent transition-colors lg:block'
                  }
                />
              )}
            </div>
            ...
            <Card
              className={cn(
                'relative min-h-0 flex-1 overflow-hidden rounded-xl bg-sidebar p-0 lg:rounded-l-none lg:rounded-r-xl border-sidebar-border',
                isMobile && mobilePane !== 'editor' ? 'hidden' : 'flex flex-col',
              )}
            ...
            >
              {isCompareDragOver && (
                <div
                  className={
                    'pointer-events-none absolute inset-0 z-20 flex items-center justify-center border-2 border-dashed border-primary/70 bg-sidebar-accent/30 text-sm font-medium text-sidebar-accent-foreground'
                  }
                >
              ...
              <div
                className={cn(
                  'border-b border-sidebar-border px-4 font-medium text-sidebar-foreground',
                  localCompare ? 'py-2' : 'py-3',
                )}
              >
                {selected ? (
                  <div className={'flex min-w-0 items-center justify-between gap-3'}>
                    <div className={'min-w-0'}>
                      ...
                      {localCompare && (
                        <p className={'truncate text-xs text-muted-foreground'}>Comparing with {localCompare.name}</p>
                      )}
                    ...
                    <div className={'min-w-full font-mono text-xs leading-5'}>
                      {selectedLines.map((line, index) => (
                        <div key={`${selected.valueUuid}-${index}`} className={'flex'}>
                          <span className='select-none bg-sidebar px-3 text-right text-muted-foreground w-12 shrink-0'>
                            {index + 1}
                          </span>
                          <code className='block w-full whitespace-pre-wrap px-3'>
                            {renderHighlightedLine(line, formatKey)}
                          </code>
                        </div>
                      ))}
                    </div>
