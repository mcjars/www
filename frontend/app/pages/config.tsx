...
const renderTreeNode = (node: TreeNode, depth: number, path: string, setSelected: (value: ConfigItem) => void) => {
    ...
    <div className={'w-full'}>
      {folderEntries.length > 0 && (
        <Accordion
          type={'multiple'}
          className={'w-full'}
          defaultValue={folderEntries.map(([folderName]) => `${path}/${folderName}`)}
        >
            ...
            return (
              <AccordionItem key={folderPath} value={folderPath} className={'border-none'}>
                <AccordionTrigger className={'px-0 py-0 hover:no-underline'}>
                  <div
                    className={'flex w-full items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-accent/10'}
                    style={{ paddingLeft: depth * 16 }}
                  >
                    <div className={'flex min-w-0 items-center gap-3'}>
                      <div
                        className={
                          'flex h-8 w-8 ml-2 items-center justify-center rounded-md border border-border/60 bg-muted/40 text-muted-foreground'
                        }
                      >
                ...
                <AccordionContent className={'pt-1 pb-2'}>
                  <div className={'space-y-1'}>{renderTreeNode(childNode, depth + 1, folderPath, setSelected)}</div>
                </AccordionContent>
              </AccordionItem>
            );
          })}
        ...
        {node.files.map((config) => {
          ...
          return (
            <div
              ...
              className={
                'group w-full cursor-pointer touch-pan-y rounded-lg px-2 py-2 transition-colors hover:bg-accent/10'
              }
              style={{ paddingLeft: depth * 16 }}
            >
              <div className={'flex items-center justify-between gap-3'}>
                <div className={'min-w-0 flex items-center gap-3'}>
                  <div
                    className={
                      'flex h-8 w-8 ml-2 items-center justify-center rounded-md border border-border/60 bg-muted/40 text-muted-foreground group-hover:bg-muted'
                    }
                  >
      ...
      <div className={'mb-4 flex items-center justify-between'}>
        <div>
          ...
          <p className={'text-muted-foreground text-sm'}>
            {configs ? `${configs.length} configs loaded` : 'Loading configs...'}
          </p>
        </div>
      </div>
              ...
              <Card key={type.identifier} className={'mb-2 overflow-hidden p-0'}>
                <AccordionItem value={typeId} className={'border-none'}>
                  ...
                  <AccordionContent className={'border-t px-2 py-2'}>
                    <div className={'space-y-1'}>{renderTreeNode(tree, 0, typeId, setSelected)}</div>
                  </AccordionContent>
                </AccordionItem>
              </Card>
            );
