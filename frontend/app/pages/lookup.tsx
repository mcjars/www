...
export default function PageLookup() {
  ...
  const typeData = useMemo(
    () =>
      Object.values(types ?? {})
        .flat()
        ?.find((t) => t.identifier === jarDropBuild?.type),
    [types, jarDropBuild],
  );
  ...
  const configDropMatchTypeData = useMemo(
    () =>
      Object.values(types ?? {})
        .flat()
        .find((t) => t.identifier === configDropMatch?.from),
    ...
    <>
      ...
      {!configDropMatches ? (
        ...
        <div ref={viewerRef} className={'w-full h-full pb-4 flex flex-col overflow-hidden'}>
          <div
            className={
              'mb-2 flex flex-col md:flex-row md:items-center md:justify-between sticky top-0 z-20 bg-background py-1 gap-2'
            }
          >
            <div>
              <h1 className={'text-xl md:text-2xl font-semibold'}>Config Diff Viewer</h1>
              <p className={'text-muted-foreground text-sm'}>
                Matched {configDropMatches.configs.length} best configs from your uploaded file.
              </p>
            </div>
          ...
          <Card className={'p-2 mb-2'}>
            <div className={'flex flex-col min-[480px]:flex-row min-[480px]:items-center gap-2'}>
              ...
              <Select
                value={String(configDropMatchIndex)}
                onValueChange={(value) => setConfigDropMatchIndex(Number(value))}
              >
                <SelectTrigger className={'w-full'}>
                  <div className={'flex flex-row items-center min-w-0'}>
                    {configDropMatchTypeData?.icon && (
                      <img
                        src={configDropMatchTypeData.icon}
                        alt={configDropMatchTypeData.name}
                        className={'h-5 w-5 rounded-md mr-2 shrink-0'}
                      />
                    )}
                ...
                <SelectContent>
                  {configDropMatches.configs.map((config, index) => {
                    const configTypeData = Object.values(types ?? {})
                      .flat()
                      .find((t) => t.identifier === config.from);
                    return (
                      <SelectItem key={`${config.build?.uuid ?? config.from}-${index}`} value={String(index)}>
                        {configTypeData?.name ?? config.from}{' '}
                        {config.build?.versionId ? `- ${config.build.versionId}` : ''}
                      </SelectItem>
                    );
                  })}
                </SelectContent>
      ...
      <Drawer
        open={isDragging || Boolean(jarDropBuild)}
        onOpenChange={(open) => {
          ...
          {jarDropBuild ? (
            <div className={'flex flex-row justify-between items-center p-2'}>
              <div className={'flex flex-row'}>
                <img
                  src={typeData?.icon ?? 'https://s3.mcjars.app/icons/vanilla.png'}
                  alt={jarDropBuild.type ?? undefined}
