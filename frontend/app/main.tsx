import React from 'react';
import ReactDOM from 'react-dom/client';
import { BrowserRouter, Route, Routes } from 'react-router-dom';
import { QueryParamProvider } from 'use-query-params';
import { ReactRouter6Adapter } from 'use-query-params/adapters/react-router-6';
import { AppSidebar } from '@/components/app-sidebar.tsx';
import { ThemeProvider } from '@/components/theme-provider.tsx';
import { SidebarProvider, SidebarTrigger } from '@/components/ui/sidebar.tsx';
import { Toaster } from '@/components/ui/toaster.tsx';
import PageTypeConfig from '@/pages/{type}/config.tsx';
import PageTypeStatistics from '@/pages/{type}/statistics.tsx';
import PageTypeVersions from '@/pages/{type}/versions.tsx';
import Page404 from '@/pages/404.tsx';
import PageConfig from '@/pages/config.tsx';
import PageIndex from '@/pages/index.tsx';
import PageJobStatus from '@/pages/job-status.tsx';
import PageLookup from '@/pages/lookup.tsx';
import PageOrganizations from '@/pages/organizations.tsx';

import '@/global.css';

ReactDOM.createRoot(document.getElementById('root')!).render(
  <React.StrictMode>
    <BrowserRouter>
      <QueryParamProvider adapter={ReactRouter6Adapter}>
        <ThemeProvider defaultTheme={'dark'}>
          <SidebarProvider>
            <AppSidebar />
            <main className={'relative h-screen md:w-[calc(100vw-var(--sidebar-width))] w-screen md:pt-2 pt-14 px-2'}>
              <SidebarTrigger className={'left-2 top-2 absolute md:hidden'} />
              <Routes>
                <Route path={'/'} element={<PageIndex />} />
                <Route path={'/configs'} element={<PageConfig />} />
                <Route path={'/lookup'} element={<PageLookup />} />
                <Route path={'/job-status'} element={<PageJobStatus />} />
                <Route path={'/organizations'} element={<PageOrganizations />} />
                <Route path={'/:type/config'} element={<PageTypeConfig />} />
                <Route path={'/:type/versions'} element={<PageTypeVersions />} />
                <Route path={'/:type/statistics'} element={<PageTypeStatistics />} />
                <Route path={'*'} element={<Page404 />} />
              </Routes>
              <Toaster />
            </main>
          </SidebarProvider>
        </ThemeProvider>
      </QueryParamProvider>
    </BrowserRouter>
  </React.StrictMode>,
);
