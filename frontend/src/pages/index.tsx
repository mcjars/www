...
export default function PageIndex() {
  ...
  const typeLookups = useMemo(
    () =>
      mergeLessThanPercent(
        Object.entries(typeLookupsRaw ?? {}).map(([label, data]) => ({
          label,
          total: data[typeLookupsType === 'uniqueIps' ? 'uniqueIps' : 'total'],
        })),
      ),
  ...
  const versionLookups = useMemo(
    () =>
      mergeLessThanPercent(
        Object.entries(versionLookupsRaw ?? {}).map(([label, data]) => ({
          label,
          total: data[versionLookupsType === 'uniqueIps' ? 'uniqueIps' : 'total'],
        })),
  ...
  const requestVersionStatsMonthMergedPercents = useMemo(
    () =>
      mergeLessThanPercent(
        Object.entries(requestVersionStatsMonthRaw?.requests ?? {})
          .map(([label, days]) => ({
            label,
            total: days.reduce(
              (acc, data) => acc + data[requestVersionStatsMonthType === 'uniqueIps' ? 'uniqueIps' : 'total'],
              0,
            ),
  ...
  const requestVersionStatsAllTime = useMemo(
    () =>
      mergeLessThanPercent(
        Object.entries(requestVersionStatsAllTimeRaw?.requests ?? {})
          .map(([label, data]) => ({
            label,
            total: data[requestVersionStatsAllTimeType === 'uniqueIps' ? 'uniqueIps' : 'total'],
          }))
          .sort((a, b) => b.total - a.total),
      ),
  ...
  return (
    <div>
      <Alert className={'mb-2'}>
        <AlertDescription>
          Welcome to <span className={'font-semibold'}>MCJars</span>! Use the sidebar on the left to navigate. This site
          will never have ads, and is open source on{' '}
          <a
            href={'https://github.com/mcjars/www'}
      ...
      <div className={'grid gap-2 md:grid-cols-[repeat(auto-fit,minmax(30rem,1fr))] w-full'}>
        <Card className={'p-4 min-w-40 flex flex-row items-center justify-between'}>
          ...
          <div className={'flex flex-col text-right items-end'}>
            <h1 className={'text-xl font-semibold'}>{stats?.builds || <Skeleton className={'w-20 h-7'} />}</h1>
        ...
        <Card className={'p-4 min-w-40 flex flex-row items-center justify-between'}>
          ...
          <div className={'flex flex-col text-right items-end'}>
            <h1 className={'text-xl font-semibold'}>{stats?.hashes || <Skeleton className={'w-20 h-7'} />}</h1>
      ...
      <div className={'my-2 grid xl:grid-cols-2 grid-cols-1 gap-2 w-full pb-2'}>
        ...
        <Card className={'p-4 h-[500px]'}>
          <div className={'absolute left-0 w-full top-0 flex flex-row items-center justify-between p-2 z-10'}>
            ...
            <div className={'flex flex-row flex-wrap items-start justify-end self-start'}>
              ...
              <Select
                ...
                onValueChange={(value) => setRequestVersionStatsMonthType(value)}
              >
          ...
          <div className={'absolute left-0 bottom-0 flex flex-row items-center justify-between p-2 pb-1 z-10'}>
            <Select
              value={String(requestVersionStatsMonthYear ?? new Date().getFullYear())}
              onValueChange={(value) => setRequestVersionStatsMonthYear(Number(value))}
            >
            ...
            <Select
              value={String(requestVersionStatsMonthMonth ?? new Date().getMonth() + 1)}
              onValueChange={(value) => setRequestVersionStatsMonthMonth(Number(value))}
            >
            ...
            <Select
              value={requestVersionStatsMonthDisplay ?? 'pie'}
              onValueChange={(value) => setRequestVersionStatsMonthDisplay(value)}
            >
          ...
          {!requestVersionStatsMonth?.length ||
          requestVersionStatsMonth.reduce((acc, { total }) => acc + total, 0) === 0 ? (
            requestVersionStatsMonthRaw ? (
              <div className={'w-full h-full flex flex-row items-center justify-center'}>
              ...
              {!requestVersionStatsMonthDisplay || requestVersionStatsMonthDisplay === 'pie' ? (
                ...
                <BarChart
                  data={Object.values(requestVersionStatsMonth)[0].days.map(({ day }) => ({
                    day,
                    ...Object.fromEntries(
                      requestVersionStatsMonth.map(({ label, days }) => [
                        label,
                        days.find((d) => d.day === day)?.[
                          requestVersionStatsMonthType === 'uniqueIps' ? 'uniqueIps' : 'total'
                        ] ?? 0,
                      ]),
                    ),
                  }))}
        ...
        <Card className={'p-4 h-[500px]'}>
          <div className={'absolute left-0 w-full top-0 flex flex-row items-center justify-between p-2 z-10'}>
            ...
            <div className={'flex flex-row flex-wrap items-start justify-end self-start'}>
              ...
              <Select
                ...
                onValueChange={(value) => setRequestVersionStatsAllTimeType(value)}
              >
                <SelectTrigger className={'w-[8em] ml-1'}>
                  <SelectValue placeholder={'Total'} />
                </SelectTrigger>
          ...
          {!requestVersionStatsAllTime?.length ? (
            ...
            <ChartContainer
              config={{}}
              className={'w-full md:h-[calc(100%-5rem)] h-[calc(100%-10rem)] absolute bottom-8 -left-4'}
            >
              <PieChart accessibilityLayer>
                <ChartTooltip content={<ChartTooltipContent />} />
                <Pie
