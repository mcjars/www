...
export default async function apiGetBuilds(
  type: string,
  version: string,
  options?: ApiGetBuildsOptions,
): Promise<PaginatedResponse<PartialMinecraftBuild>> {
  ...
  const { data } = await axios.get(
    `${BASE_URL}/api/v3/builds/types/${type.toUpperCase()}/versions/${version}?${params.toString()}`,
  );
  const buildsContainer =
    data?.builds && typeof data.builds === 'object' && !Array.isArray(data.builds) ? data.builds : undefined;
