...
const ResponsiveTooltip: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ children }) => {
  ...
  if (isMobile) {
    ...
    return (
      <Popover open={open} onOpenChange={setOpen}>
        {triggerChildren && <PopoverTrigger onClick={() => setOpen(!open)}>{triggerChildren}</PopoverTrigger>}
        {contentChildren && (
          <PopoverContent align='center' sideOffset={4} className={cn(tooltipStyles, 'w-auto')}>
            {contentChildren}
          </PopoverContent>
        )}
      </Popover>
...
export { ResponsiveTooltip, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger };
